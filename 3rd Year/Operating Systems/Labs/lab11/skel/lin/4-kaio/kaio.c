...
static void do_io_async(void)
{
	...
	DIE(iocb == NULL, "malloc error");

	piocb = (struct iocb **) malloc(n_files * sizeof(*piocb));
	DIE(piocb == NULL, "malloc error");
	...
	DIE(n_aio_ops < 0, "io_submit error");
