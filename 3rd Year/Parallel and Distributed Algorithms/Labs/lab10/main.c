	...
	for (int k = 0; k < CONVERGENCE_COEF; k++) {
		...
		*/
		...
		for (int i = 0; i < num_neigh; i++) {
			...
			int neigh_leader = 0;
			MPI_Recv(&neigh_leader, 1, MPI_INT, MPI_ANY_SOURCE, 1, MPI_COMM_WORLD, NULL);
...
int ** get_topology(int rank, int nProcesses, int * parents, int leader) {
	...
	for (int i = 0; i < nProcesses; i++) {
		if (rank == parents[i]) {
			for (int j = 0; j < nProcesses; j++) {
				MPI_Recv(vTopology[j], nProcesses, MPI_INT, i, 2, MPI_COMM_WORLD, NULL);
	...
	for (int i = 0; i < nProcesses; i++) {
		if (parents[rank] != -1)
			MPI_Send(topology[i], nProcesses, MPI_INT, parents[rank], 2, MPI_COMM_WORLD);
	}

	/* TODO4: Daca nu suntem liderul, asteptam topologia completa de la parinte  */
	if (rank != leader) {
		for (int i = 0; i < nProcesses; i++) {
			MPI_Recv(topology[i], nProcesses, MPI_INT, parents[rank], 2, MPI_COMM_WORLD, NULL);
		}
	}
	...
	for (int i = 0; i < num_neigh; i++) {
		if (neigh[i] != parents[rank]) {
			for (int j = 0; j < nProcesses; j++) {
				MPI_Send(topology[j], nProcesses, MPI_INT, neigh[i], 2, MPI_COMM_WORLD);
			}
		}
	} 
