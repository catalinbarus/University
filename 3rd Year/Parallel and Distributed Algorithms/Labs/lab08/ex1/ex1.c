...
int main (int argc, char *argv[])
{
    ...
    if (rank == 0) {
        ...
        MPI_Send(&random_number, 1, MPI_INT, 1, 55, MPI_COMM_WORLD);
        MPI_Recv(&recv_num, 1, MPI_INT, numtasks - 1, 55, MPI_COMM_WORLD, NULL);
        ...
        MPI_Recv(&recv_num, 1, MPI_INT, rank - 1, 55, MPI_COMM_WORLD, NULL);
        recv_num += 2;
        printf("Process rank #%d has received %d and sent %d\n", rank, recv_num - 2, recv_num);
        MPI_Send(&recv_num, 1, MPI_INT, 0, 55, MPI_COMM_WORLD);
        ...
        recv_num += 2;
        printf("Process rank #%d has received %d and sent %d\n", rank, recv_num - 2, recv_num);
        MPI_Send(&recv_num, 1, MPI_INT, rank + 1, 55, MPI_COMM_WORLD);
    }
