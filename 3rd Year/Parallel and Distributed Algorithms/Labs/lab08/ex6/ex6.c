...
int main (int argc, char *argv[])
{
    ...
    if (new_rank == 0) {
        MPI_Send(&new_rank, 1, MPI_INT, new_rank + 1, old_rank / GROUP_SIZE, custom_group);
        MPI_Recv(&recv_rank, 1, MPI_INT, new_size - 1, old_rank / GROUP_SIZE, custom_group, MPI_STATUS_IGNORE);
    } else if (new_rank == new_size - 1) {
        MPI_Recv(&recv_rank, 1, MPI_INT, new_rank - 1, old_rank / GROUP_SIZE, custom_group, MPI_STATUS_IGNORE);
        MPI_Send(&new_rank, 1, MPI_INT, 0, old_rank / GROUP_SIZE, custom_group);
    } else {
        MPI_Recv(&recv_rank, 1, MPI_INT, new_rank - 1, old_rank / GROUP_SIZE, custom_group, MPI_STATUS_IGNORE);
        MPI_Send(&new_rank, 1, MPI_INT, new_rank + 1, old_rank / GROUP_SIZE, custom_group);
    }
