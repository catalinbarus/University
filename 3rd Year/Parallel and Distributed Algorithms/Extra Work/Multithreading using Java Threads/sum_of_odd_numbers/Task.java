...
public class Task extends Thread {
    ...
    public void run() {
        ...
        */
        ...
        if (id == 0) {
            ...
            for (int i = 0; i < array_size; i++) {
                if (Testapd.bit_array[i] > nextNumber) {
                    ...
                    Testapd.oddNrs.add(Testapd.array[i]);
        ...
        start = (int) (id * (double) Testapd.oddNrs.size() / nr_threads);
        end = (int) Math.min((id + 1) * (double) Testapd.oddNrs.size() / nr_threads, Testapd.oddNrs.size());
        for(int i = start; i < end; i++) {
            Testapd.sumOdd += Testapd.oddNrs.get(i);
        }
        ...
        if(id == 0) {
            System.out.println(Testapd.sumOdd);
        }
