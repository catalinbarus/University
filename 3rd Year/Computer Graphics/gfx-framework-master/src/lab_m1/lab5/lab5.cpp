...
void Lab5::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        // Modify first and third person views
        /*if (window->KeyHold(GLFW_KEY_C)) {
            fieldOfView += deltaTime * cameraSpeed;

            if (orthoProjMode) {
                projectionMatrix = glm::perspective(RADIANS(fieldOfView), window->props.aspectRatio, ZNear, ZFar);
            }
        }*/
        ...
        // Change width/height of the projection windows (in ortographic view)
