...
void ping(int sockfd, struct in_addr ip_addr, int count)
{
	...
	for (int i = 0; i < count; i++) {
		...
		printf("ping icmp_seq=%d ttl=%d time=%f\n", ricmp_hdr->un.echo.sequence, rip_hdr->ttl, time_elapsed);
...
void traceroute(int sockfd, struct in_addr ip_addr)
{
	...
	while(1) {
		...
		ip_hdr->id = htons(getpid() & 0xFFFF);
		ip_hdr->frag_off = 0;
		ip_hdr->ttl = iterations;
		ip_hdr->protocol = 1;
		get_interface_ip(sockfd, IFNAME, &ip_hdr->saddr); //convert my IP to 32bit integer
		ip_hdr->daddr = ip_addr.s_addr;
		ip_hdr->check = 0;
		ip_hdr->check = checksum(ip_hdr, sizeof(struct iphdr));
		iterations++;
        ...
        ip_source.s_addr = rip_hdr->saddr;
    	char *ip_rec = inet_ntoa(ip_source);

    	printf("%s\n", ip_rec);

		if(icmp_hdr->type == 11 || iterations > 20){
