...
void run_client(int sockfd) {
    ...
    while (read(STDIN_FILENO, buf, BUFLEN - 1) > 0 && !isspace(buf[0])) {
        ...
        if (bytes_send < 0) {
            perror("[CLIENT] Error at sending data to server.\n");
            close(sockfd);
        }
        ...
        if (bytes_received < 0) {
            perror("[CLIENT] Error at receving data from server.\n");
            close(sockfd);
        }
...
int main(int argc, char* argv[])
{
    ...
    if (sockfd < 0) {
        ...
        perror("[CLIENT] Error while trying to connect to socket.\n");
        close(sockfd);
        return 1;
    }
    ...
    if (con < 0) {
        perror("[CLIENT] Error while trying to connect to server.\n");
        close(sockfd);
        return 1;
    }
