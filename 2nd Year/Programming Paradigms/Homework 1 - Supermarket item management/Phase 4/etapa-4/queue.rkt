...
(define (queue-empty? q)
  (cond
    ((and (stream-empty? (queue-left q)) (null? (queue-right q))) #t)
    (else #f)))
...
(define (rotate left right Acc)
  (cond
    ((and (stream-empty? left) (equal? 1 (length right)))
...
(define (enqueue x q)
  (cond
    ((equal? (queue-size-l q) (queue-size-r q))
     (make-queue (rotate (queue-left q) (append (list x) (queue-right q)) empty-stream)
                 '() (+ (queue-size-l q) (+ (queue-size-r q) 1)) 0))
    (else (make-queue (queue-left q) (append (list x) (queue-right q)) (queue-size-l q) (+ 1 (queue-size-r q))))))
...
(define (dequeue q)
  (cond
    ((< (- (queue-size-l q) 1) (queue-size-r q))
     (make-queue (rotate (stream-rest (queue-left q)) (queue-right q) empty-stream) '()
                 (+ (- (queue-size-l q) 1) (queue-size-r q)) 0))
    (else (make-queue (stream-rest (queue-left q)) (queue-right q) (- (queue-size-l q) 1) (queue-size-r q)))))
