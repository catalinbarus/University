...
(define (queue-empty? q)
  (cond
    ((and (null? (queue-left q)) (null? (queue-right q))) #t)
    (else #f)))
...
(define (enqueue x q)
  (make-queue (queue-left q) (append (list x) (queue-right q)) (queue-size-l q) (+ 1 (queue-size-r q))))
...
(define (dequeue q)
  (cond
    ((null? (queue-left q)) (make-queue (append (queue-left q) (cdr (reverse (queue-right q)))) '() (- (queue-size-r q) 1) 0))
