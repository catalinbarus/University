...
(define (min-tt-helper counters index tt)
  (cond
    ...
    ((< (counter-tt (car counters)) tt) (min-tt-helper (cdr counters) (counter-index (car counters)) (counter-tt (car counters))))
    ((and (equal? (counter-tt (car counters)) tt) (< (counter-index (car counters)) index))
     (min-tt-helper (cdr counters) (counter-index (car counters)) (counter-tt (car counters))))
    (else (min-tt-helper (cdr counters) index tt))))

(define (min-tt counters)
  (min-tt-helper counters (counter-index (car counters)) (counter-tt (car counters))))
...
(define (serve requests C1 C2 C3 C4)
  ...
  (define (check-right-counter1 name n-items)
    (cond
      ((equal? 1 (car (min-tt (list C1 C2 C3 C4)))) (serve (cdr requests) (add-to-counter C1 name n-items) C2 C3 C4))
      ((equal? 2 (car (min-tt (list C1 C2 C3 C4)))) (serve (cdr requests) C1 (add-to-counter C2 name n-items) C3 C4))
      ((equal? 3 (car (min-tt (list C1 C2 C3 C4)))) (serve (cdr requests) C1 C2 (add-to-counter C3 name n-items) C4))
      (else (serve (cdr requests) C1 C2 C3 (add-to-counter C4 name n-items)))
      ))

  (define (check-right-counter2 name n-items)
    (cond
      ((equal? 2 (car (min-tt (list C2 C3 C4)))) (serve (cdr requests) C1 (add-to-counter C2 name n-items) C3 C4))
      ((equal? 3 (car (min-tt (list C2 C3 C4)))) (serve (cdr requests) C1 C2 (add-to-counter C3 name n-items) C4))
      (else (serve (cdr requests) C1 C2 C3 (add-to-counter C4 name n-items)))
      ))

  (define (distribute name n-items)
    (cond
      ((<= n-items ITEMS) (check-right-counter1 name n-items))
      (else (check-right-counter2 name n-items))
      ))

  (define (add-delay index minutes)
    (cond
      ((equal? 1 index) (serve (cdr requests) (tt+ C1 minutes) C2 C3 C4))
      ((equal? 2 index) (serve (cdr requests) C1 (tt+ C2 minutes) C3 C4))
      ((equal? 3 index) (serve (cdr requests) C1 C2 (tt+ C3 minutes) C4))
