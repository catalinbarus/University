...
(define (deep-rev L)
  (cond
    ((null? L) '())
    ((number? (car L)) (append (deep-rev (cdr L)) (list (car L))))
...
(define (num_of_bases n L)
  (cond
    ((null? L) 0)
    ((palindrome? (num->base n (car L))) (+ 1 (num_of_bases n (cdr L))))
    (else (num_of_bases n (cdr L)))
    ))
...
(define (first-b-pal start b)
  (cond
    ((<= b 0) 0)
    ((>= (num_of_bases start '(2 3 4 5 6 7 8 9 10)) b) start)
    (else (first-b-pal (+ 1 start) b))
  ))
