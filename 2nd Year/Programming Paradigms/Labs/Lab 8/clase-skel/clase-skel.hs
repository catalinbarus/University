...
emptyBoard = Board $ replicate 9 Nimic
...
place playerChoice (x, y) (Board b) = 
    let n = x * 3 + y
        cell
            | playerChoice == 'X' = X
            | otherwise = Zero
    in Board $ take n b ++ [cell] ++ drop (n + 1) b
...
instance Show TicTacToe
    where 
        show (Board board) = 
            let first = concatMap show $ take 3 board
...
instance Ord TicTacToe
    where 
        (Board b1) <= (Board b2) = 
            let moves = length . filter (/= Nimic)
            in moves b1 <= moves b2 where
