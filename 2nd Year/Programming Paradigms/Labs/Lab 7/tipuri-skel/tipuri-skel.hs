...
dotV (V a1 b1 c1) (V a2 b2 c2) = a1 * a2 + b1 * b2 + c1 * c2
...
inorder :: BST a -> [a]
inorder BSTNil = []
inorder (BSTNode elem left right) = (inorder left) ++ [elem] ++ (inorder right)
...
foldlTree f acc (BSTNode value left right) =
  let newAcc = foldlTree f (f acc value) left
  in foldlTree f newAcc right
