...
permutari4([A, B, C, D], Permutation) :- length(Permutation, 4),
    member(A, Permutation), member(B, Permutation), member(C, Permutation), member(D, Permutation).
...
alternare(_, _, 0, []).
alternare(A, B, Size, [A | Rest]) :- ReducedSize is Size - 1, Size > 0,
    alternare(B, A, ReducedSize, Rest).
...
sublista(_,_) :- false.
sublista([], []).
sublista([First|Rest], [First|Sublist]) :- sublista(Rest, Sublist).
...
extract([], _, _, []).
extract(_, _, 0, []).
extract([First|Rest], 0, End, [First|Range]) :- 
	ReducedEnd is End-1, extract(Rest, 0, ReducedEnd, Range).
extract([_|Rest], Start, End, Range) :- 
	ReducedStart is Start - 1, ReducedEnd is End - 1, extract(Rest, ReducedStart, ReducedEnd, Range).
