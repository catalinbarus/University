...
safeTaran([]).
safeTaran([_]).
safeTaran(SafeCineEPeMalOpus) :- sort(SafeCineEPeMalOpus, [lup, varza]).
...
safeMisionari(0, _).
safeMisionari(Misionari, Canibali) :- Misionari > 0, Misionari >= Canibali.
...
next_state(misionari, state(Mal1, MisMal1, CanMal1), state(Mal2, MisMal2, CanMal2)) :-
        opus(Mal1, Mal2),
        boat(MB, CB), MB =< MisMal1, CB =< CanMal1,
        MisMal2 is 3 - MisMal1 + MB, CanMal2 is 3 - CanMal1 + CB,
        OM2 is 3 - MisMal2, OC2 is 3 - CanMal2,
        safeMisionari(MisMal2, CanMal2),
