...
myConcat([],L,L).
myConcat([First|Rest],L2,[First|Sol]) :- myConcat(Rest, L2, Sol).
...
myReverse([],[]).
myReverse([First|Rest],RevList):- myReverse(Rest,RevTemp), myConcat(RevTemp,[First],RevList).
...
myReverseAcc([], L, L).
myReverseAcc([First|Rest],Acc,L):- myReverseAcc(Rest,[First|Acc],L).
...
factorial(Num, Fact):-
        Num > 0,
        PrevNum is Num - 1,
        factorial(PrevNum, PrevFact),
        Fact is Num * PrevFact.
...
setIntersection([], _, []).
setIntersection([First|Rest], L2, [First|RestInt]):-member(First, L2),
                               setIntersection(Rest, L2, RestInt).
setIntersection([First|Rest], L2, RestInt):- \+member(First, L2),
                               setIntersection(Rest, L2, RestInt).
