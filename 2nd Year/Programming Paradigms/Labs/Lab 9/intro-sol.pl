...
solve(L1, L2, R) :- findall(X, (member(X, L1), member(Y, L2), X mod Y =:= 0, \+ (member(Z, L2), Z \= Y, X mod Z =:= 0)), R).

solve1(L1, L2, R) :- bagof(X, X^(member(X, L1), member(Y, L2), X mod Y =:= 0, \+ (member(Z, L2), Z \= Y, X mod Z =:= 0)), R).
