...
%% au_pusca(?ListaNume)
au_pusca(_) :- false.
au_pusca(ListaNume) :- findall(Suspect, inarmat(Suspect, pusca), ListaNume).
...
au_arma(_, _) :- false.
au_arma(Arma, ListaNume) :- findall(Suspect, inarmat(Suspect, Arma), ListaNume).

%% au_marca/2
%% au_marca(?Marca, ?ListaNume)
au_marca(_, _) :- false.
au_marca(Marca, ListaNume) :- findall(Suspect, conduce(Suspect, Marca), ListaNume).
...
%% arme_bmw(?ListaArme)
arme_bmw(_) :- false.
arme_bmw(ListaArme) :- bagof(Arma, Nume^suspect(Nume:bmw:Arma), ListaArme).
