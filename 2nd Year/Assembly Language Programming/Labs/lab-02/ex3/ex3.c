...
void reversePic(Picture *pic)
{
	...
	aux->height = pic->height;
	aux->width = pic->width;
	aux->pix_array = malloc(aux->height * sizeof(*(aux->pix_array)));

	for (int i = 0; i < aux->height; ++i)
		aux->pix_array[i] = malloc(aux->width * sizeof(*(*(aux->pix_array))));
	for(i = 0; i<pic->width; i++)
	{
		aux->pix_array[k][i] = pic->pix_array[k][i];
		pic->pix_array[k][i] = pic->pix_array[pic->height-k][i];
		pic->pix_array[pic->height-k][i] = aux->pix_array[k][i];
		k++;
	}
...
void colorToGray(Picture *pic)
{
	...
	for(i = 0; i<pic->height; i++)
		...
		{
			pic->pix_array[i][i].R = 0.3 * pic->pix_array[i][i].R;
			pic->pix_array[i][i].G = 0.59 * pic->pix_array[i][i].G;
			pic->pix_array[i][i].B = 0.11 * pic->pix_array[i][i].B;
		}
}
